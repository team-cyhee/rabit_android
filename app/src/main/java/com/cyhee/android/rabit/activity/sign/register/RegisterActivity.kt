...
class RegisterActivity : AppCompatActivity(), RegisterContract.View {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        registerBtn.setOnClickListener {
            ...
            val birth: Date? = birthText.text.toString().let {
                if(it.isNotBlank())
                    formatter.parse(it)
                null
            }
    ...
    private fun intentListener() {
        regType = intent.getStringExtra("type")?.apply {
            ...
            pwCard.visibility = View.GONE
            pw2Card.visibility = View.GONE
            token = intent.getStringExtra("token")
        }

        if(regType == null) {
            emailCard.visibility = View.VISIBLE
            pwCard.visibility = View.VISIBLE
            pw2Card.visibility = View.VISIBLE
        }
    }
    ...
    private fun initValidation() {
        ...
        // username
        observables.add(usernameText
                .textChanges()
                .map {
                    if(it.matches("[A-Za-z\\d]{4,20}".toRegex()))
                        true
                    else {
                        usernameText.error = "4-20 글자의 영소문자"
                        false
                    }
                }
        )
        // name
        observables.add(nameText
                .textChanges()
                .map {
                    if(it.length <= 30)
                        true
                    else {
                        nameText.error = "30 글자 이하"
                        false
                    }
                }
        )
